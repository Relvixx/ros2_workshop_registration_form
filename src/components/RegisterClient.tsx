...
/** Lightweight inline WhatsApp glyph (brand icons are not in lucide-react). */
    ...
    <div className="mx-auto w-full max-w-2xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      {step === "done" && confirmResult && (
        <section aria-labelledby="done-heading" className="mt-6 rounded-3xl border border-line bg-card p-6 text-center shadow-card sm:p-10">
          ...
          {/* Optional next step — shown only after successful confirmation. */}
          <div className="mx-auto mt-6 max-w-sm rounded-2xl border border-line bg-paper px-5 py-5 text-left">
            <p className="font-display text-base font-bold text-navy-950">Stay Updated</p>
            <p className="mt-1 text-sm leading-relaxed text-ink-600">{WORKSHOP.whatsappNote}</p>
            <a
              href={WORKSHOP.whatsappGroupUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-3 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#0e7a3d] px-5 text-[15px] font-bold text-white shadow-card transition hover:bg-[#0b6331]"
            >
              <WhatsAppIcon />
              {WORKSHOP.whatsappCtaLabel}
            </a>
            <p className="mt-2 text-xs text-ink-500">Optional — joining has no effect on your registration.</p>
          </div>
        </section>
      )}
    </div>
